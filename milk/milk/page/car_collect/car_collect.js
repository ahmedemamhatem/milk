frappe.pages['car-collect'].on_page_load = function (wrapper) {
    ...
    // Initialize Frappe Controls
    const driver_control = frappe.ui.form.make_control({
        parent: $('#driver-container'),
        df: { fieldtype: 'Link', options: 'Driver', fieldname: 'driver', placeholder: 'اختر السائق', fetch_if_empty: true, min_query_length: 0 },
        render_input: true
    });
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        frappe.call({
            method: 'milk.milk.utils.insert_car_collection',
            args: { data: JSON.stringify({ driver, warehouse, quantity, date, morning, evening, milk_type }) },
