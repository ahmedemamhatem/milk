...
def get_detail_columns():
    return [
        ...
        {"label": "Supplier", "fieldname": "party", "fieldtype": "Link", "options": "Supplier", "width": 200},
...
def get_detail_data(f):
    ...
    if f.get("supplier"):
        ...
        party_condition = "and gle.party = %(supplier)s"

    # Opening per party (needed if showing all)
    ...
    # For parties with no transactions in range but with opening, we still want to show an opening line
    parties_seen = set([t.party for t in txns])
    for p, opening in opening_map.items():
        if p not in parties_seen:
            # emit opening-only row
            data.append({
    ...
    for r in txns:
        if r.party != current_party:
            # new party section: add opening row
...
def get_summary_columns():
    return [
        {"label": "Supplier", "fieldname": "party", "fieldtype": "Link", "options": "Supplier", "width": 220},
...
def get_summary_data(f):
    ...
    # Parties are union of those with opening or movement
